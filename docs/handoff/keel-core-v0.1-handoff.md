...
- 렌더러 셸: Orca 토큰(Tailwind 4 + shadcn/ui new-york-v4, MIT 고지 보존)과 타이틀바·좌 사이드바·중앙 탭(분할 트리)·우 패널 컴포넌트, 탭·분할 Zustand 스토어, webview 호스트, `KeelShell`·`useKeel()` 공개 API(`src/renderer/index.ts`).
...
- webview에 포커스가 있을 때는 셸 단축키(`Mod+B/L/W`, `Mod+Shift+]/[`)가 동작하지 않는다. 다만 close 없는 앱 메뉴를 적용해 Cmd+W가 창을 닫지는 않는다(후속: Orca식 `before-input-event`로 웹뷰 포커스 중에도 셸 단축키를 전달).
...
- 설계 대비 편차: 상태 표시줄 미구현(설계 §3), `split(direction)`이 설계의 `split(id, direction)` 대신 활성 그룹을 분할한다.
- 최종 리뷰에서 남긴 후속 확인 항목: Cmd+Q 시 마지막 ≤300ms UI 상태 유실 가능(before-quit flush 필요), 분할 칸에서 웹 클릭 시 활성 그룹 미전환(webview focus 이벤트 미연결), `canGoBack` NOT_ATTACHED 경쟁(dom-ready 후 등록 필요), `repairTabs`가 손상 레이아웃에서 root 그룹을 지울 수 있음, 하이드레이션 폭 미클램프, `.npmrc` shamefully-hoist는 pnpm 전용(npm 경고), `build`의 `rm -rf`는 Windows 소비 불가, `keel.d.ts`의 무효한 webview 타입 증강, `.titlebar-left` 폭 미지정, TabBar 접근성 이름, IPC senderFrame URL 미검사, `fetchAsApp`가 set-cookie 헤더를 렌더러에 넘김, 셸 기본 세션 권한 핸들러 없음, `mailto:` 링크 차단.
