...
- [docs] 골든노트·GateLab·Deuce 세 앱을 같은 방식으로 만들 수 있는지 각 저장소·운영 응답·Orca 공개 소스로 확인하고, Orca 셸 이식 범위·`<webview>` 탭·정책 계층·앱 계약·확장 지점·앱별 적용·배포 제안·검증 기준을 담은 설계 문서를 작성했다(2026-09-13 감독 승인).
...
- [fix] webview src를 최초 URL로 고정해 게스트 탐색마다 일어나던 이중 로드·SPA 전체 리로드를 없앴다.
- [fix] 탭이 하나뿐인 그룹을 분할할 때 원래 칸이 비지 않도록 활성 탭을 옮기지 않게 했다.
- [fix] close 항목 없는 앱 메뉴를 설정해 웹 탭 포커스 상태의 Cmd+W가 창을 닫지 않게 했다.
- [fix] THIRD_PARTY_NOTICES.md를 배포 파일에 포함해 Orca MIT·Geist OFL 고지가 설치본에 실리게 했다.
- [docs] 계획 대비 편차를 기록했다: 상태 표시줄은 v0.1에 구현하지 않았고, useKeel().split은 설계의 split(id, direction) 대신 활성 그룹을 분할하는 split(direction)이다.
